...
void init_audio(Media *audio)
{
    ...
    audio->playCond = SDL_CreateCond();    
    audio->playMutex = SDL_CreateMutex();    
...
int decode_audio(Media *audio, uint8_t *audio_buf, int buf_size, double *pts_ptr)
{
	...
	if(pkt->data)
	    av_free_packet(pkt);
	/* next packet */
	if(get_from_queue(&audio->rawDataBuf, pkt) < 0) {
	    break;
	}
	...
	if(pkt->pts != AV_NOPTS_VALUE) {
	    audio->clk = av_q2d(audio->pCodecCtx->time_base)*pkt->pts;
	}
    }
}
